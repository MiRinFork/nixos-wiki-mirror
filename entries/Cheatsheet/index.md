...
When packages are managed using <a href="Flakes" class="wikilink" title="Flakes">Flakes</a>, store paths to them can be retrieved using `nix eval --inputs-from`, like this:

``` console
$ nix eval --inputs-from "$flake_path" --raw "$input#$package"
```

For instance, when packages are managed using <a href="Home_Manager" class="wikilink" title="Home Manager">Home Manager</a> using standard configuration, store path to the <a href="Git" class="wikilink" title="Git">Git</a> package can be retrieved using this command:
...
The following command will cross compile the `tinc` package for the aarch64 CPU architecture from the current system architecture (e.g. x86_64 if you are running a x86_64 system).

``` console
$ nix-build '<nixpkgs>' -A pkgsCross.aarch64-multiplatform.tinc
```

There are alternative ways to specify and build cross compiled packages, see <a href="Cross_Compiling" class="wikilink" title="Cross Compiling">Cross Compiling</a>.
...
- <a href="Garbage_Collection" class="wikilink" title="Garbage Collection">Garbage Collection</a>
- <a href="NFS#Nix_store_on_NFS" class="wikilink" title="Nix store on NFS">Nix store on NFS</a>

<a href="Category:Cookbook" class="wikilink" title="Category:Cookbook">Category:Cookbook</a> <a href="Category:Software" class="wikilink" title="Category:Software">Category:Software</a>
