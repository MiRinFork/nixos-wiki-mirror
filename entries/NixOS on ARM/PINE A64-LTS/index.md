...
<!-- Source page: NixOS on ARM/PINE A64-LTS -->
...
<thead>
<tr>
<th colspan="2" class="title"><p>PINE A64-LTS</p></th>
</tr>
</thead>
<tbody>
<tr>
<td colspan="2"><figure>
<img src="Pine-a64-lts.jpg" title="A PINE A64-LTS with eMMC." width="256" />
<figcaption>A PINE A64-LTS with eMMC.</figcaption>
</figure></td>
</tr>
<tr>
...
<td><p>SD, eMMC, SPI NOR Flash</p></td>
</tr>
<tr>
...
<hr />
<ol>
<li id="fn1"><a href="https://github.com/NixOS/nixpkgs/pull/32553#issuecomment-373915787">https://github.com/NixOS/nixpkgs/pull/32553#issuecomment-373915787</a><a href="#fnref1" class="footnote-back" role="doc-backlink">↩︎</a></li>
</ol>
</section>
...
U-boot support [has been added 2018-03-18](https://github.com/NixOS/nixpkgs/pull/32553#issuecomment-373915787). The bootloader with SPL can be downloaded from this location:

- [Nixpkgs unstable builds on hydra.](https://hydra.nixos.org/job/nixpkgs/trunk/ubootPine64LTS.aarch64-linux)

## Installation instructions

Flashing U-Boot to the SD card can be skipped if it is installed to the <a href="#SPI_NOR_flash" class="wikilink" title="SPI NOR flash">SPI NOR flash</a>
...
Follows a simplified drawing of the edge of the Pine A64-LTS, with two locations to tap for serial.[^1][^2]

<figure>
<img src="Pine-A64-LTS_partial-pinout.png" title="Pine-A64-LTS_partial-pinout.png" width="860" />
<figcaption>Pine-A64-LTS_partial-pinout.png</figcaption>
</figure>

<table>
<thead>
<tr>
<th colspan="2" style="background: var(--color-inverted)"><p>EXP Connector</p></th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td colspan="2" style="background: var(--color-inverted)"><p>Euler "e" Connector</p></td>
</tr>
<tr>
<td><p>Pin</p></td>
<td><p>Function</p></td>
...
<td><p>Ethernet</p></td>
<td><ul>
<li>Up to and including 4.19, ethernet seems to have troubles. The interface is detected but does not work right.</li>
<li>Verified only starting with 4.20 (rc4 tested), ethernet seems to work sometimes. It may fail with <code>dmesg</code> messages like <code>dwmac-sun8i 1c30000.ethernet eth0: Could not attach to PHY</code> and <code>dwmac-sun8i 1c30000.ethernet eth0: stmmac_open: Cannot attach to PHY (error: -19)</code>.</li>
</ul></td>
</tr>
<tr>
<td><p>HDMI</p></td>
<td><ul>
<li>Starting with 4.20 (rc4 tested), HDMI output works.</li>
<li>Up to and including 4.19, HDMI output does not work.</li>
</ul></td>
</tr>
</tbody>
</table>
...
- [CallMeFoxie/linux@v4.14-pine64](https://github.com/CallMeFoxie/linux/tree/v4.14-pine64)
- [Pine64 Forum post by CallMeFoxie](https://forum.pine64.org/showthread.php?tid=4931)
...
It appears that Ethernet with the [pine64 clusterboard](https://www.pine64.org/clusterboard/) requires [a slightly modified device tree to work](https://forum.pine64.org/showthread.php?tid=10432). There's some conflicting information about exactly what is needed to get Ethernet working but currently, mainline linux (5.4.58 at the time of testing) plus a modified device tree is sufficient to get a full speed 1gbps connection.
...
The easiest method to trigger [FEL mode](http://linux-sunxi.org/FEL) is to not have previously flashed the SPI NOR flash with a valid bootloader, and to remove all storage devices used to boot. The Pine A64-LTS will fallback to FEL mode.

To connect the Pine A64-LTS to the computer in FEL mode, you will need a USB A-to-A (male to male) cable, and plug to the upper USB port[^3].
...
It is possible, through using a U-Boot bootloader built from the u-boot-sunxi tree, to write to the SPI NOR flash. The [ayufan-pine64/bootloader-build](https://github.com/ayufan-pine64/bootloader-build) has such a build. Using the released `.img` files, it is possible to write their custom build or erase their custom build. From their custom build, it is possible to write to the SPI NOR flash using the `sf` command[^4].
...
- [Official product page](https://www.pine64.org/?page_id=46823)

[^1]: <http://wiki.pine64.org/index.php/PINE_A64-LTS/SOPine_Main_Page#PINE_A64-LTS_Board_Features>

[^2]: <http://files.pine64.org/doc/Pine%20A64%20Schematic/Pine%20A64%20Pin%20Assignment%20160119.pdf>

[^3]: <http://linux-sunxi.org/Pine64#FEL_mode>

[^4]: <https://github.com/ayufan-pine64/bootloader-build/blob/d7e891cbee8a559ea50cf25ee18aa9d7b4ea9d58/blobs/flash-spi.cmd#L11-L13>
