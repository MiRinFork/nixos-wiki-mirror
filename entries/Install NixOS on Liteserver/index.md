...
<!-- Source page: Install NixOS on Liteserver -->

This guide describes how to install NixOS on [liteserver](https://liteserver.nl/), paying special attention to networking using liteserver's provided IPV6 subnets.
...
Once you have a VPS with liteserver, follow their [manual installation guide](https://clients.liteserver.nl/index.php?rp=/knowledgebase/43/How-do-I-install-or-reinstall-my-VPS.html). The most appropriate ISO is generally the minimal one. By the end of this process, the 'Boot Order' under Settings/VPS Configuration should have 'CDROM1' (or something similar) *first* (on top) and then 'DISK1' (or something similar) *second*. This is important.

## NixOS installation process

Liteserver provides shell access to your VPS with an in-browser terminal emulator. The icon is a stylised 'VNC' under the 'server information' tab on the page for your VPS. Click the icon to open the terminal emulator. Now follow the [NixOS manual](https://nixos.org/manual/nixos/stable/), with specific guidance below. Given the above process of installing on the VPS, you should begin at [Manual Installation](https://nixos.org/manual/nixos/stable/#sec-installation-manual). As the manual suggests, you should run:
...
Finally, you must configure DNS nameservers. The recommended nameservers for liteserver are listed at the bottom of [this knowledgebase article](https://clients.liteserver.nl/index.php?rp=/knowledgebase/43/How-do-I-install-or-reinstall-my-VPS.html), under the heading 'Recommended IPv6 DNS Resolvers'. Edit the file `/etc/resolv.conf` (e.g. with `nano /etc/resolv.conf`) and for every DNS resolver address add a new line of the following form:
...
For more about this process, see [the baeldung article](https://www.baeldung.com/linux/dns-resolv-conf-file) on DNS configuration, and the man page `resolv.conf(5)`
...
Liteserver's VPSs are non-UEFI systems (i.e. 'legacy'). Follow the manual for this type. The manual assumes that `/dev/sda` exists (as well as friends like `/dev/sda1`, `/dev/sda2`, etc.). On VPSs like Liteserver provides, `/dev/sda` *doesn't* exist, but `/dev/vda` plays the same role (see [this baeldung article](https://www.baeldung.com/linux/vda-vs-sda) for a good explanation of this). Accordingly, substitute `/dev/vda`, `/dev/vda1`, `/dev/vda2`, etc for `/dev/sda`, `/dev/sda1`, `/dev/sda2`, etc in the manual's instructions.
