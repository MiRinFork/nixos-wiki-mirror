...
All other options aside from "listen" and "root" can be configured via SWS's [TOML config file](https://static-web-server.net/configuration/config-file/) like this:
...
NixOS includes an ACME client which will automatically request and renew TLS certificates for you. See the <a href="ACME" class="wikilink" title="ACME">ACME</a> page for more info. Here is an example of how to integrate the ACME client with SWS:
...
To check the logs, use [journalctl](https://www.man7.org/linux/man-pages/man1/journalctl.1.html), such as `journalctl -u static-web-server.service --since="30 min ago"`. You may need to increase the log level like so:
...
<a href="Category:Cookbook" class="wikilink" title="Category:Cookbook">Category:Cookbook</a> <a href="Category:Server" class="wikilink" title="Category:Server">Category:Server</a>
