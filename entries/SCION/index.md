...
Users of `scion ping` and other SCION applications must be in the `scion` group in order to access the dispatcher socket in `/run/shm/dispatcher/default.sock`. You can add users to the SCION group in your NixOS configuration as follows
...
The [scion-apps](https://github.com/netsec-ethz/scion-apps) are not included by default even when `services.scion.enable = true`. You can add them to your NixOS configuration as follows
...
3\. Fill out the following form, making sure to select `SCION installation from sources` ![](SCIONLab_AS_Form.png "SCIONLab_AS_Form.png")4. Click `Create AS`
...
Once connected to SCIONLab, you should be able to ping one of the core AS from <https://www.scionlab.org/topology.png>, as below
...
Additionally, you can try using the `scion-apps` as described in the upstream documentation <https://docs.scionlab.org/content/apps/>.

<a href="Category:Networking" class="wikilink" title="Category:Networking">Category:Networking</a> <a href="Category:Applications" class="wikilink" title="Category:Applications">Category:Applications</a>

[^1]: <https://en.wikipedia.org/w/index.php?title=SCION_(Internet_architecture)&oldid=1154623316>
