...
<!-- Source page: Packaging/Binaries -->
...
Which indicates the interpreter, <a href="#The_Dynamic_Loader" class="wikilink" title="the dynamic loader">the dynamic loader</a> in this case, could not be located.
...
See [this commit](https://github.com/NixOS/nixpkgs/commit/ea5787ad5291ee1c131326cb9c9fec03d359edff), or the example in method 5 of [this answer](https://unix.stackexchange.com/a/522823), for more details.
...
However sometimes this is not enough, more hardcoded paths may be scattered all over the place. For this you may need to set up an FHSUserEnv, a Linux Standard Base style directory structure with a final `chroot` call to fixate all paths. For a tutorial on how to use this technique, check out [Anders Papittos blog post on installing debian packages in nixos](https://web.archive.org/web/20240125095859/http://reflexivereflection.com/posts/2015-02-28-deb-installation-nixos.html).

### Starting Point

We want to package a tool called "MasterPDFEditor", the package for debian can be found at [1](http://get.code-industry.net/public/master-pdf-editor-4.3.10_qt5.amd64.deb) ([archive.org mirror](https://web.archive.org/web/20170914112947/http://get.code-industry.net/public/master-pdf-editor-4.3.10_qt5.amd64.deb)). This tutorial assumes you run nixos-17.09 or later (for `nix-index` to work).
...
Packaging is straight forward. We just have to add all the steps we did into a simple derivation file. We call it `package.nix` and store it in the checked out nixpkgs repository at `pkgs/by-name/ma/master-pdf-editor` The content looks like this:
...
Because we created a derivation which is meant to be called by callPackage we can build the package now only via: `nix-build -E '((import `<nixpkgs>` {}).callPackage (import ./package.nix) { })' --keep-failed --no-out-link`

## Creating a Pull Request

With this new package you can <a href="Pull_request" class="wikilink" title="create a pull request for nixpkgs">create a pull request for nixpkgs</a>. Be aware that binary distributions are frowned upon if the source is available.
...
This means: process `357679` is trying to open file `0600-k_cache17.bin` either from the hard-coded path in `/opt/brother/Printers/hll3210cw/inf/lut` or from the current workdir. So, as a quick fix, we could change the working directory of the wrapped binary with [makeWrapper](https://github.com/NixOS/nixpkgs/blob/master/pkgs/build-support/setup-hooks/make-wrapper.sh):
...
- <a href="Steam#steam-run" class="wikilink" title="steam-run">steam-run</a> as a quick way to run binaries

<a href="Category:Tutorial" class="wikilink" title="Category:Tutorial">Category:Tutorial</a> <a href="Category:nixpkgs" class="wikilink" title="Category:nixpkgs">Category:nixpkgs</a>
