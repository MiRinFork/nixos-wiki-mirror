...
[linux-hardened](https://github.com/anthraxx/linux-hardened) is a Linux kernel with additional hardening patches applied. It can be built from source, but the kernel must be kept up to date to receive security patches. <a href="Binary_Cache" class="wikilink" title="Binary Cache">Binary Cache</a> can be used to distribute compiled kernel binaries to multiple computers. You can check for latest releases [here](https://github.com/anthraxx/linux-hardened/releases).
...
let
  linux_hardened = pkgs.callPackage ({ fetchFromGitHub, buildLinux, ... }:
    buildLinux rec {
      version = "6.18.44-hardened1";
      extraMeta.branch = "6.18";
      hash = "sha256-ncV1k+cLKfWV36oCAkmKb8LnHRJTe6M3IByfcDi8VLI=";
      src = fetchFromGitHub {
        inherit hash;
        owner = "anthraxx";
        repo = "linux-hardened";
...
in
  pkgs.linuxPackagesFor linux_hardened
```
...
boot.kernelPackages = let
    pkgs = import <nixpkgs> {};
  in
    import ./linux-hardened.nix { inherit pkgs lib; };
```
