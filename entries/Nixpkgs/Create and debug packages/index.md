...
The code for nix packages is managed in the [nixpkgs/pkgs](https://github.com/NixOS/nixpkgs/tree/master/pkgs) repository. NixOS services, and other system configuration options are managed in [nixpkgs/nixos](https://github.com/NixOS/nixpkgs/tree/master/nixos).
...
2.  Clone your fork `git clone --filter=`[`blob:none`](blob:none)` https://github.com/YOURNAME/nixpkgs.git`
3.  Hack hack hack
...
2.  Look in nixpkgs for a package with a similar build process to use as reference. For example, if you're packaging a project written in Go, find a package for an existing Go application. Each language has its own supporting Nix functions and a more or less standard way of dealing with things. For example, [Go](https://nixos.org/manual/nixpkgs/stable/#sec-language-go) has `buildGoModule`. [Rust](https://nixos.org/manual/nixpkgs/stable/#rust) has `buildRustPackage`. [Python](https://nixos.org/manual/nixpkgs/stable/#python) has `buildPythonApplication`. [Node.js](https://nixos.org/manual/nixpkgs/stable/#node.js) has `node2nix`, `yarn2nix`, etc. There are also specific functions for wrapping e.g. [GNOME](https://nixos.org/manual/nixpkgs/stable/#sec-language-gnome) applications (`wrapGAppsHook`), or [Qt](https://nixos.org/manual/nixpkgs/stable/#sec-language-qt) apps (`libsForQt5`, `wrapQtAppsHook`). Refer to the [language support chapter in the nixpkgs manual](https://nixos.org/manual/nixpkgs/stable/#chap-language-support).
...
8.  For large projects with long compile times, you can use `nix-shell` instead to [run the individual phases](https://nixos.org/manual/nixpkgs/stable/#sec-building-stdenv-package-in-nix-shell).
9.  At this stage, you may encounter some build quirks of the project. Compile-time errors will hopefully explain what you're missing. For example [micromamba needs a specialized build of libsolv](https://github.com/NixOS/nixpkgs/blob/643ce4bd0f057bc0b90f0faebeb83a3b14f01674/pkgs/tools/package-management/micromamba/default.nix#L6-L10).
...
It is "good practice" to format packages in a way that following changed will create as minimal diffs as possible. The formatter [nixfmt](https://github.com/NixOS/nixfmt) can be used for that.
...
You can test some "good practices" in a package with [nixpkgs-hammering](https://github.com/jtojnar/nixpkgs-hammering). But before applying the recommendations you should read the given [explanations](https://github.com/jtojnar/nixpkgs-hammering/tree/main/explanations).
...
- <a href="Generic_Algorithm_on_Doing_Packaging" class="wikilink" title="Generic Algorithm on Doing Packaging">Generic Algorithm on Doing Packaging</a>
- [Fundamentals of Stdenv](https://nixos.org/guides/nix-pills/fundamentals-of-stdenv.html) in Nix Pills
- [Developing with nix-shell](https://nixos.org/guides/nix-pills/developing-with-nix-shell.html) in Nix Pills

<a href="Category:Development" class="wikilink" title="Category:Development">Category:Development</a> <a href="Category:NixOS" class="wikilink" title="Category:NixOS">Category:NixOS</a> <a href="Category:Nixpkgs" class="wikilink" title="Category:Nixpkgs">Category:Nixpkgs</a> <a href="Category:Guide" class="wikilink" title="Category:Guide">Category:Guide</a>
