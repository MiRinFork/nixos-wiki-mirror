...
<translate> To install Kitty, add it to either the system-wide `environment.systemPackages` in `/etc/nixos/configuration.nix` or to the user-specific `home.packages` in `~/.config/nixpkgs/home.nix`.[^2] </translate>
...
<translate> NixOS does not include a native module for Kitty. For basic Kitty configuration on NixOS, you need to manually follow the steps from Kitty's documentation.[^3] </translate>

<translate> If you're using Home Manager, you can enable Kitty with a simple configuration: </translate>
...
<translate> For more advanced Home Manager configuration, you can specify various Kitty settings: </translate>
...
You can use <a href="Stylix" class="wikilink" title="Stylix">Stylix</a>[^4] to theme Kitty by enabling its built-in integration:[^5] </translate>
